...
| **[canvas](skills/canvas/SKILL.md)** | One live page per topic the user and the agent edit together — a localhost daemon hot-swaps only the sections that changed, stores the user's click-to-annotate notes as typed, and writes them straight to disk. Graphics-first; the page is the reply, chat stays near-silent. The round runs in the session when the user asks; nothing wakes an agent in the background. |
...
| **[jev](skills/jev/SKILL.md)** | Ask TypeSafe's System One model for a scored second opinion about a piece of state, through `cred`. Advisory only — it never gates. Carries the rubrics for the real gates and a protocol for finding out whether it helps: ask at each gate, log whether it changed the answer, review after ten. |
